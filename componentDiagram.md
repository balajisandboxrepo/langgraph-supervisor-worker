```mermaid
%%{init: {'flowchart': {'curve': 'linear'}, 'theme': 'base', 'primaryColor': '#2b5c8f', 'fontSize': '16px', 'fontFamily': 'arial', 'primaryBorderColor': '#333333', 'lineColor': '#FF6B6B', 'tertiaryLineColor': '#FF6B6B', 'background': '#ffffff', 'mainBkg': '#ffffff', 'secondBkg': '#f0f0f0'}}%%
graph LR
    %% External Data Sources
    A["<b>Webhooks / API</b>"]
    B["<b>Enterprise Applications</b>"]
    
    %% Prefect Orchestration
    subgraph Prefect ["<b>Prefect Orchestration Layer</b>"]
        PF["<b>Flow: Orchestrator</b>"]
        T1["Task: Ingest & Preprocess"]
        T2["<b>Task: Run LangGraph</b>"]
        T3["Task: Postprocess & Notify"]
        PF --> T1 --> T2 --> T3
    end
    
    %% LangGraph Agentic Layer
    subgraph LangGraph ["<b>LangGraph Agentic Layer</b>"]
        Start(["<b>START</b>"])
        State["<b>State Manager</b>"]
        Sup["<b>Supervisor</b>"]
        W1["<b>Worker:</b><br/>DocumentExtract"]
        W2["<b>Worker:</b><br/>Object Detection"]
        W3["<b>Worker:</b><br/>Classification"]
        End(["<b>END</b>"])
        
        Start --> State --> Sup
        Sup --> W1
        Sup --> W2
        Sup --> W3
        W1 --> End
        W2 --> End
        W3 --> End
    end
    
    %% External Services
    subgraph Ext ["<b>External Services & LLMs</b>"]
        APIcalls["<b>API Calls</b>"]
        VectorDB["<b>Vector DB</b><br/>FAISS"]
        LLM["<b>LLM</b><br/>Qwen / OpenAI"]
        Slack["<b>Slack</b><br/>Email"]
    end
    
    %% Connections
    A --> PF
    B --> T1
    T2 --> Start
    W1 -.-> APIcalls
    W2 -.-> VectorDB
    Sup -.-> LLM
    T3 --> Slack
    
    classDef prefect fill:#2b5c8f,stroke:#fff,stroke-width:3px,color:#fff,font-size:14px;
    classDef langgraph fill:#138a72,stroke:#fff,stroke-width:3px,color:#fff,font-size:14px;
    classDef external fill:#f39c12,stroke:#fff,stroke-width:3px,color:#000,font-size:13px;
    classDef source fill:#555,stroke:#fff,stroke-width:2px,color:#fff,font-size:13px;
    classDef core fill:#d32f2f,stroke:#fff,stroke-width:4px,color:#fff,font-size:15px;
    
    class PF,T1,T2,T3 prefect;
    class Start,State,Sup,W1,W2,W3,End langgraph;
    class APIcalls,VectorDB,LLM,Slack external;
    class A,B source;
    class Sup core;
```